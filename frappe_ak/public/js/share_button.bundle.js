...
function _dd_add_button(frm, templates) {
    ...
    // Add "Doc Designer Shares" button to view all shares for this document
    const sharesLabel = __("Doc Designer Shares");
    if (frm.custom_buttons && frm.custom_buttons[sharesLabel]) return;
...
function _dd_show_shares_panel(frm) {
    frappe.call({
        ...
        args: {
            ...
            fields: [
                "name", "template", "status", "recipient_email",
                "share_url", "creation", "submitted_at", "view_count",
            ],
...
function _dd_render_shares_dialog(frm, shares) {
    ...
    for (const s of shares) {
        ...
        const badge =
            '<span style="display:inline-block;padding:2px 10px;border-radius:5px;font-size:12px;' +
            "font-weight:600;border:1px solid " + sc.border + ";background:" + sc.bg + ";color:" + sc.text + ';">' +
            frappe.utils.escape_html(s.status) + "</span>";

        const date = frappe.datetime.str_to_user(s.submitted_at || s.creation);
        const email = s.recipient_email
            ? frappe.utils.escape_html(s.recipient_email)
            : '<span style="color:#9ca3af;">—</span>';

        let actions = '<div style="display:flex;gap:6px;justify-content:flex-end;">';

        // View filled document button for responded shares
        if (["Submitted", "Accepted", "Declined"].includes(s.status)) {
            actions +=
                '<button class="btn btn-xs btn-primary _dd-view-response" data-share="' +
                s.name + '">' + __("View Document") + "</button>";
        }

        // Open share record
        actions +=
            '<button class="btn btn-xs btn-default _dd-open-share" data-share="' +
            s.name + '">' + __("Details") + "</button>";
        ...
        rows +=
            ...
            '<td style="padding:10px 12px;font-size:13px;color:#6b7280;">' + date + "</td>" +
            '<td style="padding:10px 12px;text-align:center;font-size:13px;">' + (s.view_count || 0) + "</td>" +
            '<td style="padding:10px 12px;">' + actions + "</td>" +
            "</tr>";
    }

    const html =
        '<div style="font-family:var(--font-stack);overflow:auto;">' +
        '<table style="width:100%;border-collapse:collapse;">' +
        '<thead><tr style="background:#f9fafb;border-bottom:2px solid #e5e7eb;">' +
        '<th style="padding:8px 12px;text-align:left;font-size:11px;text-transform:uppercase;color:#6b7280;font-weight:600;">' + __("Status") + "</th>" +
        '<th style="padding:8px 12px;text-align:left;font-size:11px;text-transform:uppercase;color:#6b7280;font-weight:600;">' + __("Recipient") + "</th>" +
        '<th style="padding:8px 12px;text-align:left;font-size:11px;text-transform:uppercase;color:#6b7280;font-weight:600;">' + __("Date") + "</th>" +
        '<th style="padding:8px 12px;text-align:center;font-size:11px;text-transform:uppercase;color:#6b7280;font-weight:600;">' + __("Views") + "</th>" +
        '<th style="padding:8px 12px;text-align:right;font-size:11px;text-transform:uppercase;color:#6b7280;font-weight:600;">' + __("Actions") + "</th>" +
        "</tr></thead>" +
        "<tbody>" + rows + "</tbody>" +
        "</table></div>";

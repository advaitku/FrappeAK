...
def submit_response(secret_key, response_type, field_values):
    ...
    share_doc.status = response_type if response_type in ("Accepted", "Declined") else "Submitted"
    share_doc.submitted_at = now_datetime()

    if template.lock_after_submission:
        share_doc.is_locked = 1
...
def send_document_email(share_name):
    ...
    from doc_designer_ak.email_utils import send_document_email as _send
    _send(share_name)
    return {"success": True}
...
def get_preview_html(share_name):
    ...
    from doc_designer_ak.renderer import render_template

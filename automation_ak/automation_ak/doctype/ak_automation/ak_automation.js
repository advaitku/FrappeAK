	...
	refresh(frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(__("Test Automation"), function () {
				frappe.call({
					method: "automation_ak.api.automation.test_automation",
...
frappe.ui.form.on("AK Automation Condition", {
	field(frm, cdt, cdn) {
		...
		if (row.field && frm.doc.reference_doctype) {
			frappe.call({
				method: "automation_ak.api.automation.get_operators_for_field",
...
function _load_doctype_fields(frm) {
	frappe.call({
		method: "automation_ak.api.automation.get_doctype_fields",
